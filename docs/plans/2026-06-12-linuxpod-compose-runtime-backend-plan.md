...
- [ ] Define a JSONL schema for host footprint evidence that separates
  cgroup/guest memory from host physical memory.
- [ ] Test host-side sources such as `footprint`, `vmmap -summary`,
  process-tree sampling, `vm_stat` deltas, and any exposed
  Containerization/Virtualization identifiers.
- [ ] Reject sources that do not scale with guest workload, as runner-process
  RSS was rejected in the spike.
- [ ] Record skipped or blocked evidence when attribution is not reliable.
- [ ] Update documentation to avoid saying "base VM overhead is lower" unless
  a reliable host-side source proves it.

### Phase 6: Backend-Shaped Public Fixture Benchmark

- [ ] Create or reuse a public backend-shaped fixture with DB, migrate, seed,
  API, named volume, readiness, published API port, logs, status, and cleanup.
- [ ] Run LinuxPod dry-run evidence first.
- [ ] After explicit runtime approval, run one LinuxPod smoke iteration.
- [ ] If smoke succeeds, run `5` warm-image iterations.
- [ ] Stop LinuxPod replacement work and record `linuxpod-not-promising` if the
  `5` warm-image iterations miss the Docker/OrbStack Viability Gate by a wide
  margin.
- [ ] Run `10` iterations only if results pass the Docker/OrbStack Viability
  Gate and are near the competitive thresholds.
- [ ] Run `20` or more iterations only if LinuxPod appears capable of beating
  Docker/OrbStack and publication-grade evidence is useful.
- [ ] Capture RAM, CPU, disk footprint, block I/O, startup/readiness, cleanup,
...
- [ ] Write a decision note under `docs/plans/notes/` with one of:
  `linuxpod-primary`, `linuxpod-promising`, `linuxpod-multi-service-only`,
  `linuxpod-blocked`, or `linuxpod-not-promising`.
- [ ] Update `docs/plans/notes/index.md`.
- [ ] Update this plan and `docs/plans/index.md` with the next concrete todo or
  completion state.
- [ ] Update the main implementation plan only if the decision changes what an
  implementer should do next.
