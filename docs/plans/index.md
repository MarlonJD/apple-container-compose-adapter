...
| active | [LinuxPod Phase 6B Root-Cause Implementation Plan](2026-06-12-linuxpod-phase-6b-root-cause-plan.md) | `tools/apple-container-compose-adapter` | Implement only the small Phase 6B root-cause harness: span timing, cache-hit proof, preserved-state LinuxPod modes, and optional detection-only Colima/Lima/Finch comparison; do not change product direction. | Planned after the 2026-06-12 user-approved narrow hypothesis: LinuxPod memory signal was slightly better than Apple `container` DB+API, but startup/readiness and block reads made it worse as a product path. Verification must produce dry-cache JSONL, signed runtime root-cause evidence if approved, a decision note, and matching notes/index updates. |
